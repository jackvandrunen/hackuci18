...
class Vec2ValenceSet(torch.utils.data.Dataset):

    def __init__(self, path):
        with open(path) as f:
            self.data = [(torch.FloatTensor(i[0]), torch.FloatTensor([i[1][0]])) for i in json.load(f)]
...
if __name__ == '__main__':
    dataset = Vec2ValenceSet('./emovec.json')
    ...
    #model = Vec2ValenceNet(D_in, H, D_out)
    model = torch.load('valence_model1.dat')
    ...
    for epoch in range(1, 100001):
        ...
        if avg < min_loss:
            min_loss = avg
            torch.save(model, 'valence_model1.dat')
        if not epoch % 25:
